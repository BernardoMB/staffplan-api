...
exports.getStaffList = function (req, res) {
	req.getConnection(function (err, connection) {
		var query = connection.query('SELECT STAFF_ID, CONCAT_WS(" ", FIRST_NAME,MIDDLE_INITIAL,LAST_NAME) AS STAFF_NAME FROM STAFF', function (err, rows) {
			if (err) {
				return res.send({
					"error": true,
					"status": "failed",
		...
		async.waterfall([
			function (callback) {
				...
				if(req.body.ONBENCH){
					...
					connection.query('SELECT *, CONCAT_WS(" ", FIRST_NAME,MIDDLE_INITIAL,LAST_NAME) AS STAFF_NAME from '+ DBName +'.STAFF INNER JOIN STAFF_ROLE ON STAFF.STAFF_ROLE_ID = STAFF_ROLE.ROLE_ID INNER JOIN STAFF_GROUP ON STAFF.STAFF_GROUP_ID = STAFF_GROUP.GROUP_ID INNER JOIN STAFF_STATUS ON STAFF.STAFF_STATUS_ID = STAFF_STATUS.STATUS_ID INNER JOIN OFFICE ON STAFF.OFFICE_ID = OFFICE.OFFICE_ID Where STAFF_ID NOT in ( SELECT DISTINCT STAFF_ID FROM '+ DBName +'.PROJECT_PEOPLE WHERE START_DATE <= NOW() AND END_DATE >= NOW())', function (err, StaffIDList) {
				...
				if(!req.body.ONBENCH){
					var query = connection.query("SELECT STAFF.*, CONCAT_WS(' ', FIRST_NAME,MIDDLE_INITIAL,LAST_NAME) AS STAFF_NAME , STAFF_STATUS.STATUS_NAME,STAFF_ROLE.ROLE_NAME,STAFF_GROUP.GROUP_NAME, OFFICE.OFFICE_NAME FROM STAFF INNER JOIN STAFF_ROLE ON STAFF.STAFF_ROLE_ID = STAFF_ROLE.ROLE_ID INNER JOIN STAFF_GROUP ON STAFF.STAFF_GROUP_ID = STAFF_GROUP.GROUP_ID INNER JOIN STAFF_STATUS ON STAFF.STAFF_STATUS_ID = STAFF_STATUS.STATUS_ID INNER JOIN OFFICE ON STAFF.OFFICE_ID = OFFICE.OFFICE_ID " + con, function (err, rows) {
						if (err) {
							return res.send({
								"error": true,
								"status": "failed",
...
exports.getAvailableStaff = function(req,res){
	...
	req.getConnection(function (err, connection) {
		...
		if(req.body.EMPTY){
			async.waterfall([
				function (callback) {
					...
					connection.query('SELECT STAFF.*, CONCAT_WS(" ", FIRST_NAME,MIDDLE_INITIAL,LAST_NAME) AS STAFF_NAME, STAFF_ROLE.ROLE_NAME FROM STAFF LEFT JOIN STAFF_ROLE ON STAFF_ROLE.ROLE_ID = STAFF.STAFF_ROLE_ID WHERE STAFF_ID in(' + data + ')', function (err, StaffIDList) {
						if (err) {
							callback(null, err);
						} else {
							StaffIDList.forEach(element => {
		...
		// 		})
		// 	},function(data,callback){
		// 		connection.query('SELECT STAFF.*, CONCAT_WS(" ", FIRST_NAME,MIDDLE_INITIAL,LAST_NAME) AS STAFF_NAME, STAFF_ROLE.ROLE_NAME FROM STAFF LEFT JOIN STAFF_ROLE ON STAFF_ROLE.ROLE_ID = STAFF.STAFF_ROLE_ID WHERE STAFF_ROLE.ROLE_ID = "' + req.body.STAFF_ROLE_ID + '" AND STAFF_ID in(' + data + ')', function (err, StaffIDList) {
		// 			if (err) {
		// 				callback(null, err);
		// 			} else {
		// 				StaffIDList.forEach(element => {
...
exports.getAvailableStaff2 = function(req,res){
	...
	var tempQueryProject = ' AND 1 = 1';
		if(req.body.EMPTY){
			async.waterfall([
				function (callback) {
					connection.query('SELECT STAFF_ID,CONCAT_WS(" ", STAFF.FIRST_NAME,STAFF.MIDDLE_INITIAL,STAFF.LAST_NAME) AS STAFF_NAME from '+ DBName +'.STAFF Where STAFF_ID NOT in ( SELECT DISTINCT STAFF_ID FROM '+ DBName +'.PROJECT_PEOPLE)', function (err, StaffIDList) {
						if (err) {
					...
					connection.query('SELECT PROJECT_PEOPLE.*,CONCAT_WS(" ", STAFF.FIRST_NAME,STAFF.MIDDLE_INITIAL,STAFF.LAST_NAME) AS STAFF_NAME FROM '+ DBName +'.PROJECT_PEOPLE INNER JOIN STAFF ON STAFF.STAFF_ID = PROJECT_PEOPLE.STAFF_ID WHERE START_DATE >= "' + currentDate + '"', function (err, StaffIDList) {
						if (err) {
							callback(null, 'Something went wrong');
						} else {
			...
			async.waterfall([
				function (callback) {
					connection.query('SELECT *,CONCAT_WS(" ", STAFF.FIRST_NAME,STAFF.MIDDLE_INITIAL,STAFF.LAST_NAME) AS STAFF_NAME from '+ DBName +'.STAFF Where STAFF_ID NOT in ( SELECT DISTINCT STAFF_ID FROM '+ DBName +'.PROJECT_PEOPLE) AND STAFF.STAFF_ROLE_ID = "' + req.body.ROLE_ID + '"' , function (err, StaffIDList) {
						if (err) {
							callback(null, 'Something went wrong');
						} else {
							// console.log(StaffIDList);
					...
					connection.query('SELECT PROJECT_PEOPLE.*,CONCAT_WS(" ", STAFF.FIRST_NAME,STAFF.MIDDLE_INITIAL,STAFF.LAST_NAME) AS STAFF_NAME FROM '+ DBName +'.PROJECT_PEOPLE INNER JOIN STAFF ON STAFF.STAFF_ID = PROJECT_PEOPLE.STAFF_ID WHERE START_DATE >= "' + currentDate + '" AND PROJECT_ROLE_ID = "' + req.body.ROLE_ID + '"', function (err, StaffIDList) {
						if (err) {
							callback(null, 'Something went wrong');
						} else {
							const groupBystafId = _.groupBy(StaffIDList, 'STAFF_ID');
			...
			async.waterfall([
				function (callback) {
					connection.query('SELECT *,CONCAT_WS(" ", STAFF.FIRST_NAME,STAFF.MIDDLE_INITIAL,STAFF.LAST_NAME) AS STAFF_NAME from '+ DBName +'.STAFF Where STAFF_ID NOT in ( SELECT DISTINCT STAFF_ID FROM '+ DBName +'.PROJECT_PEOPLE)' + tempQueryStaff , function (err, StaffIDList) {
						if (err) {
							callback(null, 'Something went wrong');
						} else {
							// console.log(StaffIDList);
