...
exports.getStaffList = function (req, res) {
	req.getConnection(function (err, connection) {
		var query = connection.query('SELECT STAFF_ID, CONCAT_WS(" ",  (CASE WHEN PREFERRED_NAME = "" THEN FIRST_NAME WHEN PREFERRED_NAME IS NULL THEN FIRST_NAME ELSE PREFERRED_NAME END), MIDDLE_INITIAL,LAST_NAME) AS STAFF_NAME FROM STAFF', function (err, rows) {
			if (err) {
				return res.send({
					"error": true,
					"status": "failed",
		...
		async.waterfall([
			function (callback) {
				...
				if(req.body.ONBENCH){
					...
					connection.query('SELECT *, CONCAT_WS(" ",  (CASE WHEN PREFERRED_NAME = "" THEN FIRST_NAME WHEN PREFERRED_NAME IS NULL THEN FIRST_NAME ELSE PREFERRED_NAME END), MIDDLE_INITIAL,LAST_NAME) AS STAFF_NAME from '+ DBName +'.STAFF INNER JOIN STAFF_ROLE ON STAFF.STAFF_ROLE_ID = STAFF_ROLE.ROLE_ID INNER JOIN STAFF_GROUP ON STAFF.STAFF_GROUP_ID = STAFF_GROUP.GROUP_ID INNER JOIN STAFF_STATUS ON STAFF.STAFF_STATUS_ID = STAFF_STATUS.STATUS_ID INNER JOIN OFFICE ON STAFF.OFFICE_ID = OFFICE.OFFICE_ID Where STAFF_ID NOT in ( SELECT DISTINCT STAFF_ID FROM '+ DBName +'.PROJECT_PEOPLE WHERE START_DATE <= NOW() AND END_DATE >= NOW())', function (err, StaffIDList) {
				...
				if(!req.body.ONBENCH){
					var query = connection.query("SELECT STAFF.*, CONCAT_WS(' ',  (CASE WHEN PREFERRED_NAME = '' THEN FIRST_NAME WHEN PREFERRED_NAME IS NULL THEN FIRST_NAME ELSE PREFERRED_NAME END), MIDDLE_INITIAL,LAST_NAME) AS STAFF_NAME , STAFF_STATUS.STATUS_NAME,STAFF_ROLE.ROLE_NAME,STAFF_GROUP.GROUP_NAME, OFFICE.OFFICE_NAME FROM STAFF INNER JOIN STAFF_ROLE ON STAFF.STAFF_ROLE_ID = STAFF_ROLE.ROLE_ID INNER JOIN STAFF_GROUP ON STAFF.STAFF_GROUP_ID = STAFF_GROUP.GROUP_ID INNER JOIN STAFF_STATUS ON STAFF.STAFF_STATUS_ID = STAFF_STATUS.STATUS_ID INNER JOIN OFFICE ON STAFF.OFFICE_ID = OFFICE.OFFICE_ID " + con, function (err, rows) {
						if (err) {
							return res.send({
								"error": true,
								"status": "failed",
		...
		if(req.body.EMPTY){
			async.waterfall([
				function (callback) {
					...
					connection.query('SELECT STAFF.*, CONCAT_WS(" ",  (CASE WHEN PREFERRED_NAME = "" THEN FIRST_NAME WHEN PREFERRED_NAME IS NULL THEN FIRST_NAME ELSE PREFERRED_NAME END), MIDDLE_INITIAL,LAST_NAME) AS STAFF_NAME, STAFF_ROLE.ROLE_NAME FROM STAFF LEFT JOIN STAFF_ROLE ON STAFF_ROLE.ROLE_ID = STAFF.STAFF_ROLE_ID WHERE STAFF_ID in(' + data + ')', function (err, StaffIDList) {
						if (err) {
							callback(null, err);
						} else {
							StaffIDList.forEach(element => {
			...
			async.waterfall([
				function (callback) {
					...
					connection.query('SELECT STAFF.*, CONCAT_WS(" ",  (CASE WHEN PREFERRED_NAME = "" THEN FIRST_NAME WHEN PREFERRED_NAME IS NULL THEN FIRST_NAME ELSE PREFERRED_NAME END), MIDDLE_INITIAL,LAST_NAME) AS STAFF_NAME, STAFF_ROLE.ROLE_NAME, ANY_VALUE(PROJECT_PEOPLE.END_DATE) AS NEXT_AVAILABLE,(SELECT SUM(ALLOCATION) as ALLOCATION_TOTAL FROM PROJECT_PEOPLE WHERE PROJECT_PEOPLE.STAFF_ID = STAFF.STAFF_ID GROUP BY PROJECT_PEOPLE.STAFF_ID) as ALLOCATION_TOTAL  FROM STAFF LEFT JOIN STAFF_ROLE ON STAFF_ROLE.ROLE_ID = STAFF.STAFF_ROLE_ID  LEFT JOIN PROJECT_PEOPLE ON PROJECT_PEOPLE.STAFF_ID = STAFF.STAFF_ID  WHERE STAFF_ROLE.ROLE_ID = "' + req.body.ROLE_ID + '" AND STAFF.STAFF_ID in(' + data + ') GROUP BY STAFF.STAFF_ID', function (err, StaffIDList) {
						if (err) {
							callback(null, err);
						} else {
							StaffIDList.forEach(element => {
