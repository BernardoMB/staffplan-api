...
from CALENDAR
  ...
  assignmentList: (plannedId, condition) =>
    // `
    // SELECT SUM(STAFF_ALLOCATION.ALLOCATION) ALLOCATION, WEEK, YEAR, STAFF_ID
    // FROM
    //   CALENDAR
    ...
    //   AND ${condition}
    // group by STAFF_ID, CALENDAR.CALENDAR_ID, CALENDAR.WEEK, CALENDAR.YEAR
    // `,
    `
    SELECT SUM(STAFF_ALLOCATION.ALLOCATION) ALLOCATION, WEEK, YEAR, STAFF_ID
    FROM CALENDAR
          INNER JOIN STAFF_ALLOCATION ON CALENDAR.CALENDAR_ID = STAFF_ALLOCATION.CALENDAR_ID
          INNER JOIN PROJECT_STAFF ON PROJECT_STAFF.ID = STAFF_ALLOCATION.PROJECT_STAFF_ID
    WHERE CALENDAR.START_DATE >= date_add(LAST_DAY(PROJECT_STAFF.START_DATE - INTERVAL 1 MONTH), interval 1 Day)
      AND CALENDAR.END_DATE <= PROJECT_STAFF.END_DATE
      AND ${condition}
    group by STAFF_ID, CALENDAR.CALENDAR_ID, CALENDAR.WEEK, CALENDAR.YEAR
    `,
