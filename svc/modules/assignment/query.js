...
module.exports = {
  getProjectTeams: (condition) =>
    ...
    SELECT CAST(
       ...
       CASE
           WHEN PROJECT_STAFF.STAFF_ID IS NOT NULL THEN PROJECT_STAFF.ID
           ELSE PROJECT_STAFF.PLANNED_STAFF_ID
           END                     ID,
       STAFF.STAFF_ID,
       STAFF.OFFICE_ID,
       OFFICE.OFFICE_NAME,
  ...
  getProjectTeamsCalendar: (condition, startDate, endDate) =>
    ...
    select CAST(
       ...
       PROJECT_STAFF.PROJECT_ID, PROJECT_STAFF.START_DATE, PROJECT_STAFF.END_DATE,
      STAFF.STAFF_STATUS_ID, STAFF_STATUS.STATUS_NAME, SA.ID AS ALLOCATION_ID,
       PROJECT_STAFF.RESUME_SUBMITTED, PROJECT_STAFF.ID AS PROJECT_STAFF_ID,
       STAFF.STAFF_PHOTO, PROJECT_STAFF.PLANNED_STAFF_ID
from CALENDAR
         left join STAFF_ALLOCATION SA on CALENDAR.CALENDAR_ID = SA.CALENDAR_ID
         inner join PROJECT_STAFF on SA.PROJECT_STAFF_ID = PROJECT_STAFF.ID
    ...
    SELECT CURRENT.STAFF_ID
      ...
      WHERE CURRENT.END_DATE > CURDATE()
        AND FUTURE.END_DATE > CURDATE()
    ) AS GAP ON GAP.STAFF_ID = STAFF.STAFF_ID
         LEFT OUTER JOIN STAFF_STATUS
                         ON (STAFF.STAFF_STATUS_ID IS NOT NULL AND STAFF_STATUS.STATUS_ID = STAFF.STAFF_STATUS_ID)
         LEFT OUTER JOIN OFFICE
                         ON (STAFF.OFFICE_ID IS NOT NULL AND OFFICE.OFFICE_ID = STAFF.OFFICE_ID)
