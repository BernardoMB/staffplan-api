...
module.exports = {
  ...
  StaffingGap: (condition) => (
    ...
    /*`SELECT STAFF.STAFF_ID, PROJECT_STAFF.START_DATE, PROJECT_STAFF.END_DATE 
      ...
      `SELECT PROJECT_STAFF.STAFF_ID AS TOTAL
      FROM PROJECT_STAFF
      WHERE ( 
        PROJECT_STAFF.END_DATE < DATE_ADD(NOW(), INTERVAL 90 DAY) || 
        PROJECT_STAFF.START_DATE > NOW()) AND
      PROJECT_STAFF.STAFF_ID in (SELECT STAFF_ID FROM STAFF WHERE STAFF.${condition})
