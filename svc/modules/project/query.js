module.exports = {
  ...
  SELECT
    ...
    ROLE_ID
  FROM CALENDAR C
         LEFT JOIN STAFF_ALLOCATION SA ON C.CALENDAR_ID = SA.CALENDAR_ID
         LEFT OUTER JOIN PROJECT_STAFF PS ON SA.PROJECT_STAFF_ID = PS.ID
         LEFT OUTER JOIN PROJECT P ON PS.PROJECT_ID = P.PROJECT_ID
         LEFT OUTER JOIN PROJECT_STATUS ON P.PROJECT_STATUS_ID = PROJECT_STATUS.STATUS_ID
         LEFT OUTER JOIN STAFF S on PS.STAFF_ID = S.STAFF_ID
         LEFT OUTER JOIN STAFF_ROLE SR on PS.PROJECT_ROLE_ID = SR.ROLE_ID
  WHERE C.START_DATE >= SUBDATE(CURDATE(), dayofweek(CURDATE()) - 1)
  AND C.END_DATE <= DATE(CURDATE() + INTERVAL 6 MONTH)
  ORDER BY C.CALENDAR_ID, ROLE_NAME
    `,
